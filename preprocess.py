...
# ============================================================
# Canonical 30 columns we will output for ALL datasets
# ============================================================
...
# Simple US region mapping by state_code
US_REGION_MAP = {
   # West
...
def years_diff(start, end):
   """Safe difference in years between two datetimes."""
...
def preprocess_cax(df_raw: pd.DataFrame) -> pd.DataFrame:
   ...
   out["status"] = status_raw.replace({"success": "operating", "failed": "closed"})
...
# ============================================================
# Preprocess: Full companies.csv (Crunchbase master)
# ============================================================


def preprocess_companies(df_raw: pd.DataFrame) -> pd.DataFrame:
   df = df_raw.copy()
   out = pd.DataFrame(index=df.index)


   # --- Identity ---
   out["company_id"] = df["id"]
   out["company_name"] = df["name"]
   ...
   status = df["status"].astype(str).str.lower()
   out["status"] = status
   # same heuristic as other Crunchbase-like data
   out["success_label"] = np.where(status.isin(["acquired", "ipo", "operating"]), 1, 0)


   # --- Geography ---
   out["country"] = df.get("country_code")
   out["state_code"] = df.get("state_code")
   out["city"] = df.get("city")
   out["latitude"] = pd.to_numeric(df.get("lat"), errors="coerce")
   out["longitude"] = pd.to_numeric(df.get("lng"), errors="coerce")


   # Region: use US regions if country is USA, else leave NaN
   regions = []
   country_series = out["country"]
   state_series = out["state_code"]
   for cc, st in zip(country_series, state_series):
       if isinstance(cc, str) and cc.upper() == "USA":
           regions.append(US_REGION_MAP.get(st, np.nan))
       else:
   ...
   # --- Dates / ages ---
   out["founded_at"] = pd.to_datetime(df.get("founded_at"), errors="coerce")
   out["closed_at"] = pd.to_datetime(df.get("closed_at"), errors="coerce")
   out["first_funding_at"] = pd.to_datetime(df.get("first_funding_at"), errors="coerce")
   out["last_funding_at"] = pd.to_datetime(df.get("last_funding_at"), errors="coerce")
   ...
   # Age at first/last funding computed directly from dates
   out["age_at_first_funding_years"] = [
       years_diff(f, ff) for f, ff in zip(out["founded_at"], out["first_funding_at"])
   ]
   out["age_at_last_funding_years"] = [
       years_diff(f, lf) for f, lf in zip(out["founded_at"], out["last_funding_at"])
   ]


   # --- Industry / product ---
   cat = df.get("category_code").astype(str).str.lower()
   out["primary_industry"] = df.get("category_code")


   out["is_software"] = cat.str.contains("software").astype(int)
   out["is_web"] = cat.str.contains("web|internet|online").astype(int)
   out["is_mobile"] = cat.str.contains("mobile").astype(int)
   out["is_enterprise"] = cat.str.contains("enterprise|b2b|saas").astype(int)
   out["is_biotech_or_health"] = cat.str.contains(
       "biotech|health|medical|pharma|drug|diagnostic|healthcare"
   ).astype(int)


   # --- Funding ---
   out["funding_total_usd"] = pd.to_numeric(df.get("funding_total_usd"), errors="coerce")
   out["funding_rounds"] = pd.to_numeric(df.get("funding_rounds"), errors="coerce")


   # We don't have clean VC/angel separation here; use rough proxies
   out["has_VC"] = out["funding_rounds"].fillna(0).gt(0).astype(int)
   out["has_angel"] = 0  # unknown, so default to 0


   out["avg_participants"] = np.nan  # not available in this table


   # --- Team / network / scale ---
   out["num_founders"] = pd.to_numeric(df.get("relationships"), errors="coerce")
   out["employee_count"] = np.nan  # not available in companies.csv
...
# ============================================================
# Main entry point: run all preprocessors
# ============================================================
...
if __name__ == "__main__":
   ...
   companies_path = "./data/original/companies.csv"


   cax_raw = pd.read_csv(cax_path)
   cb_raw = pd.read_csv(cb_path)
   companies_raw = pd.read_csv(companies_path)


   cax_clean = preprocess_cax(cax_raw)
   cb_clean = preprocess_crunchbase(cb_raw)
   companies_clean = preprocess_companies(companies_raw)
   ...
   companies_clean.to_csv("data/cleaned/companies_preprocessed_30cols.csv", index=False)


   # Combined dataset from all three sources
   combined = pd.concat([cax_clean, cb_clean, companies_clean], ignore_index=True)
   combined.to_csv("data/cleaned/combined_startups_30cols.csv", index=False)


   print(
       "Saved cax_preprocessed_30cols.csv, "
       "crunchbase_preprocessed_30cols.csv, "
       "companies_preprocessed_30cols.csv, "
       "and combined_startups_30cols.csv"
   )
